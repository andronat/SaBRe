...
dicom)
  PROJECT_NAME="dicom"
  AFL_ARGS="-d -m 512 -i ./conf/in-dicom -P DICOM -E -K -R"
  TARGET_CONF=""
...
dicom-asan)
  ...
  AFL_ARGS="-d -t 1000 -m none -i ./conf/in-dicom -P DICOM -E -K -R"
  TARGET_CONF=""
...
dns)
  PROJECT_NAME="dns"
  AFL_ARGS="-d -m 512 -i ./conf/in-dns -P DNS -K -R"
  TARGET_CONF="-C ./conf/dnsmasq-deimos.conf"
  TARGET_BINS=("./dnsmasq")
  ;;
dns-asan)
  PROJECT_NAME="dns-asan"
  AFL_ARGS="-d -t 1000 -m none -i ./conf/in-dns -P DNS -K -R"
  TARGET_CONF="-C ./conf/dnsmasq-deimos.conf"
...
dtls)
  PROJECT_NAME="dtls"
  AFL_ARGS="-d -m 512 -i ./conf/in-dtls -P DTLS12 -q 3 -s 3 -E -K -R"
  TARGET_CONF=""
  TARGET_BINS=("./dtls-server")
  ;;
dtls-asan)
  PROJECT_NAME="dtls-asan"
  AFL_ARGS="-d -t 1000 -m none -i ./conf/in-dtls -P DTLS12 -q 3 -s 3 -E -K -R"
  TARGET_CONF=""
...
ftp)
  PROJECT_NAME="ftp"
  AFL_ARGS="-d -m 512 -i ./conf/in-ftp -x ./conf/ftp.dict -P FTP -q 3 -s 3 -E -R"
...
ftp-asan)
  ...
  AFL_ARGS="-d -t 1000 -m none -i ./conf/in-ftp -x ./conf/ftp.dict -P FTP -q 3 -s 3 -E -R"
  TARGET_CONF="./conf/fftp-deimos.conf 2200"
...
rtsp)
  PROJECT_NAME="rtsp"
  AFL_ARGS="-d -m 512 -i ./conf/in-rtsp -x ./conf/rtsp.dict -P RTSP -q 3 -s 3 -E -K -R"
  TARGET_CONF="8554"
  TARGET_BINS=("./testOnDemandRTSPServer")
  ;;
rtsp-asan)
  PROJECT_NAME="rtsp-asan"
  AFL_ARGS="-d -t 1000 -m none -i ./conf/in-rtsp -x ./conf/rtsp.dict -P RTSP -q 3 -s 3 -E -K -R"
  TARGET_CONF="8554"
