...
def gather_perf_mode():
    ...
    for fname in glob.glob(FUZZER_STAT_FILES_GLOB):
        with open(fname) as f:
            ...
            results.append(
                f"{label:55} {str(run_time):10} {str(execs_print):20} {cycles_done:5} {str(stability):10} {hangs_crashes:10} {aux:20}"
            )
            writer.writerow(
                [
                    label,
    ...
    print(f"Avg total time: {timedelta(seconds=avg_total_seconds)}")
    csv_file.close()
